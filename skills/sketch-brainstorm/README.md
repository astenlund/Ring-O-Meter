...
Both transport directions work end-to-end and the full interpretation pipeline (strokes -> composite -> multimodal subagent -> structured JSON) is functionally validated. **Outbound**: render an HTML mockup to a PDF, push to a folder on the reMarkable cloud via rmapi. **Inbound**: pull a cloud document with `rmapi get`, extract the `.rmdoc` archive, parse the per-page `.rm` files into transparent SVG overlays, composite each overlay onto its source PDF page, dispatch a fresh subagent with the composite PNGs + the vocabulary, receive a structured JSON block with `user_intent`, `design_state_delta`, and `per_page_observations`. The iter01+ loop that consumes `user_intent` to drive the next render, `design-state.md` for cross-turn memory, and bootstrap-lite (the minimum viable preamble that gets iter 00 onto the tablet) all ship today. The stroke-region Finish-turn detector and one-time calibration ceremony also ship today: `derive-calibration.sh` runs the five-dot ceremony to produce a firmware-versioned `calibration.json`, and `detect-finish-turn.sh` per-turn emits structured JSON describing whether the box is marked. Background polling now wraps the detector: `poll-tablet.sh` spawns as a backgrounded process, uses `rmapi stat` (Version + ModifiedClient) to short-circuit idle iterations, pulls + detects only on cloud-side change, and emits `READY:<NN>` on stdout for the harness Monitor stream when the user finishes a turn. The poller exits after one READY; the orchestrator respawns it with the next iter's `--cloud-doc` after pushing. What remains: STOP (End-session checkbox; the detector currently knows only Finish-turn), ERROR taxonomy with exponential backoff for transient rmapi failures, the full bootstrap dialogue (design-language briefing, lock-file check, resume-vs-fresh history list), verify-before-push (visual sanity check on rendered output before pushing), and compression (turn-by-turn rotation of design-state.md sections into archives). See `SKILL.md` for current entry-points.
...
- **Hand-off**: a Finish-turn checkbox at fixed pixel coordinates serves as the deterministic turn-boundary, mirrored on every rendered page so the user can mark Finish-turn from whichever page they are on. A stroke-region detector (`detect-finish-turn.sh`) inverse-transforms the checkbox's PDF rectangle into `.rm` coordinates using a firmware-versioned scale from `calibration.json`, hit-tests strokes from the pulled `.rm` files, and emits structured JSON. The one-time calibration ceremony (`derive-calibration.sh`) pins the scale by asking the user to mark five reference dots. The polling wrapper (`poll-tablet.sh`) spawns as a backgrounded process via `Bash(run_in_background=true)`, watches the current iter's cloud doc via `rmapi stat`, pulls + detects on change, and emits `READY:<NN>` on stdout when the box is marked. The poller exits after one READY; the orchestrator respawns with the next iter's `--cloud-doc` after pushing.
...
- **Layout contract**: pinned in `page-chrome.css` and `detect_finish_turn.py::FINISH_TURN_BOX_PDF`. The Finish-turn checkbox at `(1540, 2100)` 40x40 px on the `1620x2160` viewport, in the chrome footer of every rendered page, must not move without re-running the calibration ceremony and updating `FINISH_TURN_BOX_PDF` in lockstep (both files carry cross-reference comments). Sized to leave a 60 px sibling slot above for the future End-session checkbox.
...
- `bootstrap-session.sh` -- creates the per-session local folder skeleton and primes `design-state.md`; idempotent.
