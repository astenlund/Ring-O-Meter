...
export SKETCH_ON_TABLET_CHECK_RMAPI_OVERRIDE="$PASS_STUB_DIR/check-rmapi-setup.sh"

# Run bootstrap-session.sh into a sandboxed SKETCH_ON_TABLET_REPO_ROOT.
SKETCH_ON_TABLET_REPO_ROOT="$TMP" \
...
TODAY="$(date -u +%Y-%m-%d)"
SESSION_DIR="$TMP/.tmp/sketch-on-tablet/sessions/${TODAY}-warmup-gate"
...
INDEX="$TMP/.tmp/sketch-on-tablet/current-session.json"
...
SKETCH_ON_TABLET_REPO_ROOT="$TMP2" \
  bash "$SCRIPT_DIR/bootstrap-session.sh" \
    --slug blank-test \
    --topic "blank test"

SESSION_DIR2="$TMP2/.tmp/sketch-on-tablet/sessions/${TODAY}-blank-test"
...
if SKETCH_ON_TABLET_REPO_ROOT="$TMP3" \
     bash "$SCRIPT_DIR/bootstrap-session.sh" \
       --slug "evil/../boom" \
...
# Negative test: with no SKETCH_ON_TABLET_REPO_ROOT and PWD outside
...
if NEG_OUTPUT="$(cd "$TMP4" && unset SKETCH_ON_TABLET_REPO_ROOT && \
     bash "$SCRIPT_DIR/bootstrap-session.sh" \
...
if SKETCH_ON_TABLET_REPO_ROOT="$TMP_GATE" \
   SKETCH_ON_TABLET_CHECK_RMAPI_OVERRIDE="$STUB_DIR/check-rmapi-setup.sh" \
   bash "$SCRIPT_DIR/bootstrap-session.sh" \
...
[[ ! -d "$TMP_GATE/.tmp/sketch-on-tablet/sessions/${TODAY}-gated" ]] \
