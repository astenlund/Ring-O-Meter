...
// it walks the importing file's tree. SKETCH_ON_TABLET_NODE_HOST (set
...
function loadPlaywright() {
  const host = process.env.SKETCH_ON_TABLET_NODE_HOST;
  if (!host) {
    fail(
      'SKETCH_ON_TABLET_NODE_HOST is not set. The bash wrapper '
      + 'render-html-to-pdf.sh sets this to whichever directory contains '
      + 'node_modules/playwright (skills/sketch-on-tablet or the host '
...
async function main() {
  ...
  try {
    ...
    // sketch-on-tablet-mockup.css by the design-language briefing at
