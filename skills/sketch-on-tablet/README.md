...
- `rmapi` on `$PATH`. The skill assumes it is installed and authenticated; see [Installing rmapi for sketch-on-tablet](#installing-rmapi-for-sketch-on-tablet) below for the full setup including the security boundary.
- A host project with Playwright available. The skill resolves it from `skills/sketch-on-tablet/node_modules` (run `npm install` from the skill folder once) with a fallback to `$REPO_ROOT/web/node_modules` for in-repo incubation. Gist consumers: `npm install` in the skill folder.
...
## Installing rmapi for sketch-on-tablet

This skill assumes [rmapi](https://github.com/ddvk/rmapi) v0.0.33+ is installed and paired to your reMarkable account, and that Claude Code is configured with a PreToolUse hook that prevents Claude from reading the rmapi token. The hook intercepts any tool call whose command, file path, or grep path contains the literal substring `rmapi.conf`, and additionally canonicalizes path-field inputs via `realpath` so a symlink at a non-`rmapi.conf` path still resolves and gets blocked.
...
- **In scope:** accidental access where Claude executes a documented or inferred path to `rmapi.conf` via any tool; honest-Claude symlink bypass on `Read` / `Edit` / `Write` / `NotebookEdit` / `Grep` (closed by `realpath` canonicalization of the path field before the regex match).
- **Out of scope:** adversarially-prompted Claude using `Bash` to read the conf via indirection the hook cannot statically resolve — symlink-then-cat through a runtime-computed path, encoded path, environment-variable indirection. The `Bash` command field receives literal-substring matching only; statically resolving filesystem accesses inside a shell command is the unsolved problem that motivated the failed `permissions.deny` investigation.
- **Out of scope:** hardlinks. The hook has no inode-identity mechanism — `realpath` resolves symlinks in path components but does not detect that two distinct path strings refer to the same inode. A hardlink to `rmapi.conf` at a non-`rmapi.conf` name (created via `ln` / `link(2)` on POSIX or `mklink /H` on Windows by an adversarially-prompted Claude) is therefore not caught. Creating a hardlink requires write permission on the containing directory, so this is only a viable surface when Claude is running as the same user that owns `~/.config/rmapi/`. Mitigation belongs to the Defense-in-depth layer (separate OS user account + ACLs).
- **Defense-in-depth:** users who need a stronger boundary should run rmapi under a separate OS user account and apply file ACLs. Outside the skill's scope.
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash $HOME/.claude/skills/sketch-on-tablet/rmapi-conf-deny-hook.sh"
          }
        ]
      }
    ]
...
**Hook-path variant for in-repo incubation:** while the skill lives inside a host repo (this repo, `skills/sketch-on-tablet/`), use:

```json
"command": "bash <absolute-path-to-repo>/skills/sketch-on-tablet/rmapi-conf-deny-hook.sh"
```
...
- `rmapi-conf-deny-hook.sh`: PreToolUse hook implementation that blocks tool calls referencing the rmapi conf. Reads tool-call JSON on stdin (extracts `tool_name`, `command`, `file_path`, `path`), canonicalizes `file_path` and `path` via `realpath` (so a symlink at a non-`rmapi.conf` name still resolves and gets blocked; the `command` field stays unresolved per the Threat model section), regex-matches `rmapi.conf` case-insensitively across the haystack, exits 2 with a 4-column audit-log entry on match (`<ts>\t<tool_name>\t<context>\t<resolved_or_dash>` — `-` for direct match, resolved path for symlink-bypass blocks), exits 0 silent on no-match, exits 0 fail-open on malformed JSON. Audit log at `~/.claude/sketch-on-tablet-conf-access.log` (override via optional positional arg `$1`, used by tests; production callers pass no args and get the default). Never invoked directly by Claude; wired via `~/.claude/settings.json`.
...
- `_lib.sh` -- internal bash helpers sourced by other wrappers: rmapi auth precondition, shared Python venv bootstrap with requirements.txt drift detection, and `find_repo_root <start-dir>` (walk-upward `$_REPO_MARKER` marker discovery; sourced by `render-html-to-pdf.sh` with `$SCRIPT_DIR` and by `bootstrap-session.sh` with `$PWD`). Also declares `_REPO_MARKER` (the repo marker filename) as the single source of truth.
...
- `render/session_index.py` -- read / write `.tmp/sketch-on-tablet/current-session.json`: `read_index`, `add_session` (idempotent on session_dir), `set_active`, `increment_turns`, `SessionIndexError`. Atomic writes via `_atomic_write`.
...
- `check-poller-lock.sh` -- bash wrapper that prints one JSON line on stdout classifying `.tmp/sketch-on-tablet/poller.lock` as `absent`, `stale`, or `alive`. Always exits 0; lock states are data, not errors.
...
- `check-compression-needed.sh` -- bash wrapper printing the compression trigger JSON; sole consumer is the orchestrator's loop body step 8.6.
- `render/check_compression_needed.py` -- compression trigger logic. Exports `WATERMARK_OFFSET` (default 5) and `check_trigger(session_dir) -> dict`. Stdlib-only.
- `render/test_check_compression_needed.py` -- unit tests for the trigger logic.
- `test_check_compression_needed.sh` -- bash wrapper test.
- `write-archive.sh` -- bash wrapper for the archive writer; reads parsed JSON on stdin, takes `--session-dir`, `--turns-to-archive`, `--turns-to-keep`.
- `render/write_archive.py` -- archive writer + active-head rewriter; validates three structural invariants and performs the two-step atomic write. Exports `write(...)` and `ArchiveStructureError`. Stdlib-only.
- `render/test_write_archive.py` -- unit tests for the writer.
- `test_write_archive.sh` -- bash wrapper test.
- `parse-compress-response.mjs` -- shell-callable JSON parse + validate helper for the compression subagent's response (`{archive_content, new_active_head_content}`).
- `test_compress_parse.mjs` -- node:test cases for `parseCompressResponse`.
- `compress-prompt.md` -- prompt template for the compression subagent; orchestrator-substituted, not directly executable.
- `check-rmapi-setup.sh`: read-only three-check verifier (rmapi on PATH, conf valid, PreToolUse hook installed). Exit 0/1/2 with rationale. Safe for Claude to invoke on demand. See "Installing rmapi for sketch-on-tablet" above and the design rationale in `.claude/features/remarkable-tablet-brainstorm.md` (Transport section).
...
- `test_check_rmapi_setup.sh`: standalone bash test runner for `check-rmapi-setup.sh`. Not wired into `test.sh` (which handles Python + Node only); run manually with `bash skills/sketch-on-tablet/test_check_rmapi_setup.sh`.
