...
The set of pen gestures sketch-on-tablet recognises deterministically. Marking up the rendered mockup with these gestures lets the user communicate common edit operations without writing English instructions.
...
- **Project-local extensions** live at `.claude/sketch-on-tablet-vocab.md` in the host project and are merged into the global vocabulary on session launch (project entries override on conflict).
