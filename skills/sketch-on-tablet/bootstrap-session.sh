...
CHECK_RMAPI="${SKETCH_ON_TABLET_CHECK_RMAPI_OVERRIDE:-$SCRIPT_DIR/check-rmapi-setup.sh}"
status=0
...
TODAY="$(date -u +%Y-%m-%d)"
SESSION_DIR="$REPO_ROOT/.tmp/sketch-on-tablet/sessions/${TODAY}-${SLUG}"
...
SKETCH_ON_TABLET_REPO_ROOT="$REPO_ROOT" \
