---
name: sketch-on-tablet
description: Round-trip UI brainstorm loop with a reMarkable tablet. Use when the user wants to sketch on the tablet ("sketch on tablet", "sketch on remarkable", "sketch"). Renders HTML mockups to PDF at the tablet's viewport, pushes to the reMarkable cloud via rmapi, drives an iter-by-iter render-pull-interpret-render loop. Bootstrap covers lock-conflict check, resume-vs-fresh prompt, and design-language briefing. The poller handles transient rmapi failures with exponential backoff and emits ERROR notifications for persistent failures. Cross-turn compression ships too: the orchestrator runs compression after each push and dispatches a background subagent to archive older turns. Multi-sketch is still deferred to a follow-up slice.
---
...
`<repo-root>/.claude/sketch-on-tablet-mockup.css` on first run.
...
- `check-poller-lock.sh` -- bash wrapper that prints one JSON line on stdout classifying `.tmp/sketch-on-tablet/poller.lock` as `absent`, `stale`, or `alive`. Always exits 0; lock states are data, not errors. The full bootstrap dialogue's step 1 reads this and branches into the force-claim prompt on `alive`.
...
```
bash .claude/skills/sketch-on-tablet/render-strokes.sh \
  <rm-dir> \
  .tmp/sketch-on-tablet/test/strokes-out/
```
...
Bootstraps a Python venv at `skills/sketch-on-tablet/.venv/` on first run and installs `rmscene` via `requirements.txt`. Subsequent runs reuse the venv.
...
```
bash .claude/skills/sketch-on-tablet/render-html-to-pdf.sh \
  --topic "warmup gate UI" \
  --iteration 00 \
  --out .tmp/sketch-on-tablet/test/warmup-gate-00.pdf
```
...
The skill resolves Playwright from `skills/sketch-on-tablet/node_modules` (per-skill install, run `npm install` from the skill folder once) or falls back to `$REPO_ROOT/web/node_modules` for in-repo incubation. Gist consumers: `npm install` in the skill folder.
...
```
bash .claude/skills/sketch-on-tablet/push-to-tablet.sh \
  --pdf .tmp/sketch-on-tablet/test/seed.pdf \
...
```
bash .claude/skills/sketch-on-tablet/pull-from-tablet.sh \
  --cloud-doc Brainstorms/warmup-gate/iter01 \
  --out-dir .tmp/sketch-on-tablet/pulls/
```
...
```
STROKES_DIR="$(bash .claude/skills/sketch-on-tablet/pull-from-tablet.sh \
  --cloud-doc Brainstorms/warmup-gate/iter01 \
  --out-dir .tmp/sketch-on-tablet/pulls/)"
bash .claude/skills/sketch-on-tablet/render-strokes.sh \
  "$STROKES_DIR" .tmp/sketch-on-tablet/pulls/iter01-svgs/
```
...
1. Read `skills/sketch-on-tablet/interpret-prompt.md` (the prompt template).
2. Substitute the bracketed tokens:
   - `{COMPOSITE_PATHS}` - newline-bullet absolute paths to the composite PNGs.
   - `{VOCAB_PATH}` - absolute path to `skills/sketch-on-tablet/vocabulary.md` (and the project-local extension at `.claude/sketch-on-tablet-vocab.md` if it exists).
...
1. Read `skills/sketch-on-tablet/verify-prompt.md` (the prompt template).
2. Substitute the bracketed tokens:
...
1. Run `bash skills/sketch-on-tablet/check-compression-needed.sh <session-dir>`.
...
3. On `trigger: true`, the JSON also carries `turns_to_archive`,
   ...
   Read `skills/sketch-on-tablet/compress-prompt.md`. Substitute the
   tokens:
...
4. Dispatch via the Agent tool with `subagent_type: general-purpose`
   ...
   `node skills/sketch-on-tablet/parse-compress-response.mjs`
   (stdin = raw response; stdout = canonicalized JSON; exit 0 on valid).
5. On parse success, invoke
   `bash skills/sketch-on-tablet/write-archive.sh
...
5. Invoke: `bash skills/sketch-on-tablet/write-design-state.sh
...
8. `push-to-tablet.sh --pdf <session>/<slug>-NN.pdf --cloud-folder
   <cloud-path>/<slug>`.
8.5. `bash skills/sketch-on-tablet/update-session-index.sh increment-turn
   ...
   as turns.
8.6. Run `bash skills/sketch-on-tablet/check-compression-needed.sh <session-dir>`.
...
3. **Pixel-read fallback** -- fires only when `current_mode` is present
   ...
   3. Invoke: `bash skills/sketch-on-tablet/read-prefill.sh <path-to-pulled-pdf>`.
...
2. **Active-session check.** Read
   `.tmp/sketch-on-tablet/current-session.json` if present. If
...
4. **Cloud path resolution.** If `.tmp/sketch-on-tablet/config.json`
...
5. **Design-language briefing.** On first run in a project (when
   `<repo-root>/.claude/sketch-on-tablet-mockup.css` does not exist),
   ...
   `<repo-root>/.claude/sketch-on-tablet-mockup.css` via the Write
...
7. **Initial render and push.** Compose `mockups/<slug>-00.html`:
   ...
   `<repo-root>/.claude/sketch-on-tablet-mockup.css` when present and
...
8. **Spawn polling script.** Launch `bash poll-tablet.sh ...` as a
   ...
   writes `.tmp/sketch-on-tablet/poller.lock` at startup and updates
...
8. Full-bootstrap branches test, four checks (run each from a clean
   ...
   - **Design-language briefing.** With no
     `<repo-root>/.claude/sketch-on-tablet-mockup.css` present,
