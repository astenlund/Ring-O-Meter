...
import {publishUiOnly} from './allocHarness';
...
describe('useFrameState structural invariants', () => {
    test('buffer-flip produces a distinct latest reference each flush', async () => {
        ...
        await act(async () => {
            publishUiOnly(writer, 21, 220, 0.9);
            await waitForFlush();
        });
        ...
        await act(async () => {
            publishUiOnly(writer, 42, 330, 0.85);
            await waitForFlush();
        });
    ...
    test('unregisterReader preserves survivor latest values', async () => {
        ...
        await act(async () => {
            publishUiOnly(writerA, 21, 220, 0.9);
            publishUiOnly(writerB, 21, 440, 0.95);
            await waitForFlush();
        });
